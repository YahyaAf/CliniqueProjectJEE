...
<head>
    ...
    <title>Edit Specialty - MediCare+</title>
...
<main id="mainContent" class="p-8 ml-64">
    ...
    <!-- Page Header -->
    <div class="mb-8">
        <h1 class="text-3xl font-bold text-white mb-2">Edit Specialty</h1>
        <p class="text-white/70">Update specialty information</p>
    </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <select id="departmentId"
                        ...
                        <c:forEach var="dept" items="${departments}">
                            <option value="${dept.id}"
                                    class="bg-slate-800"
                                    <c:if test="${dept.name == specialite.departmentName}">selected</c:if>>
                                    ${dept.name}
                            </option>
                        </c:forEach>
                    </select>
                ...
                <div>
                    ...
                    <label class="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox"
                               name="isActive"
                               class="sr-only peer"
                               <c:if test="${specialite.isActive}">checked</c:if>>
                        ...
                        <span class="ms-3 text-sm font-medium text-white/90">
                            <c:choose>
                                <c:when test="${specialite.isActive}">Active</c:when>
                                <c:otherwise>Inactive</c:otherwise>
                            </c:choose>
                        </span>
                    </label>
                    <p class="text-xs text-white/50 mt-2">Toggle to activate or deactivate this specialty</p>
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="description"
                              ...
                              class="w-full px-4 py-2.5 bg-white/10 border border-white/20 rounded-lg focus:ring-2 focus:ring-indigo-400 focus:border-indigo-400 outline-none transition text-white placeholder-white/50 resize-none">${specialite.description}</textarea>
                </div>
