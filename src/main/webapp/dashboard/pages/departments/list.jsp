...
<main id="mainContent" class="p-8 ml-64">
    ...
    <!-- Page Header -->
    <div class="mb-8">
        <h1 class="text-3xl font-bold text-white mb-2">Departments Management</h1>
        <p class="text-white/70">Manage all medical departments</p>
    </div>
    ...
    <div class="bg-slate-800/40 backdrop-blur-lg border border-white/10 rounded-2xl shadow-xl p-6 mb-6 max-w-xs">
        <div class="flex items-center gap-4">
            <div class="w-14 h-14 bg-purple-500/20 rounded-xl flex items-center justify-center">
                <i class="fas fa-hospital text-purple-400 text-2xl"></i>
            </div>
            <div>
                <p class="text-white/60 text-sm">Total Departments</p>
    ...
    <div class="bg-slate-800/40 backdrop-blur-lg border border-white/10 rounded-2xl shadow-xl overflow-hidden">
        ...
        <div class="p-6 border-b border-white/10">
            <div class="flex items-center justify-between">
                <h2 class="text-xl font-bold text-white">Liste des Départements</h2>
                <div class="flex items-center gap-3">
                    ...
                    <a href="${pageContext.request.contextPath}/admin/departments?action=add"
        ...
        <c:if test="${empty departments}">
            <div class="p-12 text-center">
                <div class="w-20 h-20 bg-slate-700/50 rounded-full flex items-center justify-center mx-auto mb-4">
                    <i class="fas fa-hospital text-slate-500 text-3xl"></i>
                </div>
                <p class="text-white/60 text-lg">Aucun département trouvé.</p>
                <a href="${pageContext.request.contextPath}/admin/departments?action=add"
                ...
                <table class="w-full">
                    ...
                    <tr class="bg-slate-700/30 border-b border-white/10">
                        <th class="px-6 py-4 text-left text-sm font-semibold text-white/90">
                            <div class="flex items-center gap-2">
                                <i class="fas fa-hospital-alt text-indigo-400"></i>
                                Nom
                            </div>
                        </th>
                        <th class="px-6 py-4 text-left text-sm font-semibold text-white/90">
                    ...
                    <c:forEach var="d" items="${departments}">
                        <tr class="border-b border-white/5 hover:bg-white/5 transition-colors">
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    <div class="w-10 h-10 bg-gradient-to-br from-purple-500 to-pink-600 rounded-xl flex items-center justify-center text-white font-semibold">
                            ...
                            <td class="px-6 py-4 text-white/70 max-w-md">
                                <c:choose>
                                    <c:when test="${not empty d.description}">
                                        ${d.description}
                                    </c:when>
                                    <c:otherwise>
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-2">
                                    <a href="${pageContext.request.contextPath}/admin/departments?action=edit&id=${d.id}"
...
<script>
    ...
    if (searchInput) {
        searchInput.addEventListener('keyup', function() {
            ...
            const tableRows = document.querySelectorAll('#departmentTableBody tr');
